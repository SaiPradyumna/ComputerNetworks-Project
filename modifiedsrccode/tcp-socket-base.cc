...
TcpSocketBase::DupAck ()
{
  ...
  if (m_tcb->m_congState != TcpSocketState::CA_RECOVERY)
    ...
    {
      ++m_dupAckCount;
      //std::cout<<"HELPKdsjcbdsbvbfnvbf"<<m_dupAckCount<<"\n";
    if(m_dupAckCount>m_retxThresh && (m_dupAckCount-m_retxThresh)%2==0)
        {
          //std::cout<<m_tcb->m_congState<<"jdsfhdsgfhsdgfhvfhdsvhfdvbhfdbv\t"<<m_dupAckCount<<"\t"<<m_connected<<"\n"; 
          //std::cout<<SendPendingData(m_connected)<<"\n";
          SequenceNumber32 next;
          uint32_t s = std::max (AvailableWindow (), m_tcb->m_segmentSize);
          //std::cout<<"shfjdshfdsbhvfdhvbhfbvhfbv"<<m_tcb->m_highTxMark<<"\n";
          SendDataPacket (m_tcb->m_highTxMark,s,true);
          //NS_ASSERT (sz > 0);
        }
    }
...
TcpSocketBase::ReceivedAck (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
	...
	//uint8_t tcpflags =   & ~(TcpHeader::PSH | TcpHeader::URG);

uint32_t bdproduct;

  pcount++;
//	double time1,time2,rtttime;
...
*/
  ...
  if (pcount==1)
    {
  //  x = (tcpHeader.GetFlags () & (TcpHeader::SYN & TcpHeader::ACK));
    //std::cout<<x<<" bool\n";
    //std::cout<<pcount;
    rtt= m_rtt->GetEstimate () ;
  ...
  {
    //x = (tcpHeader.GetFlags () & (TcpHeader::SYN));
    //std::cout<<x<<" bool2\n";
   ...
   // time2=t2.GetSeconds ();
    ...
    std::cout<<"time2 "<<t2<<"f\t"<<t1<<"RTT "<<rtttime<<"\n";
  std::cout<<"time2 "<<t2<<"f\t"<<t1<<"RTT "<<t2-t1<<"\n";
  t4=((t2-t1)+(t1-t3))/2;
    bdproduct=size/t4;
    std::cout<<"d"<<bdproduct<<"\n";
//bdproduct=size/(t1-t3);
    bdproduct=bdproduct*rtttime;
    std::cout<<"Segment size: "<<size<<"BP Delay: "<<bdproduct<<" RTT "<<rtttime<<"\n";
    std::cout<<"threshold: "<<m_tcb->m_ssThresh<<"\n";
      ...
      else
        {
          ...
          // phase.
          else if (m_tcb->m_congState == TcpSocketState::CA_RECOVERY)
            {
             ...
             // m_tcb->m_cWnd=m_tcb->m_ssThresh+m_tcb->m_segmentSize;
